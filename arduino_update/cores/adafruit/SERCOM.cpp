...
void SERCOM::initUART(SercomUartMode mode, SercomUartSampleRate sampleRate, uint32_t baudrate)
{
  ...
  sercom->USART.INTENSET.reg = SERCOM_USART_INTENSET_RXC |  //Received complete
                               /*njh SERCOM_USART_INTENSET_TXC |  //Tx complete*/
...
#if defined(__SAMD51__)
void SERCOM::enableUARTtx()
{

  if (sercom->USART.CTRLB.bit.TXEN) return;

  if (sercom->USART.SYNCBUSY.bit.CTRLB) {
    //. If CTRLB is written while
    // SYNCBUSY.CTRLB is asserted, an APB error will be generated
    return;
  }
  //Setting  the enable bit to 1
  sercom->USART.CTRLB.bit.TXEN = 0x1u;

  //Wait for then enable bit from SYNCBUSY is equal to 0;
  // while(sercom->USART.SYNCBUSY.bit.CTRLB);
}
void SERCOM::disableUARTtx()
{
  //Disable UART - if half Duplex allows RX
  sercom->USART.CTRLB.bit.TXEN = 0x0u;

  //Wait for then enable bit from SYNCBUSY is equal to 0;
 // while(sercom->USART.SYNCBUSY.bit.ENABLE);
}

void SERCOM::clearUARTtxc()
{
  // cleared by writing '1' to it or by writing new data to DATA
  sercom->USART.INTENSET.bit.TXC = 0x01;
}

bool SERCOM::isUARTtxc()
{
