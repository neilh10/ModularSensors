...
void Uart::IrqTxcHandler() 
{
  #define PIN_F_D12 12
    //digitalWrite(PIN_F_D12, LOW); //Debug
    //digitalWrite(PIN_F_D12, HIGH); //Debug
    //digitalWrite(PIN_F_D12, LOW); //Debug
  //Half Duplex - Disable
  if (sercom->isUARTtxc()) 
  {
    ...
    digitalWrite(PIN_F_D12, HIGH); //Debug
    digitalWrite(PIN_F_D12, LOW); //Debug
    //sercom->disableUARTtx();
    sercom->clearUARTtxc();
    //pinMode(uc_pinTX,INPUT); set to ALTSERCOM
    //digitalWrite(PIN_F_D12, LOW); //Debug
...
size_t Uart::write(const uint8_t data)
{
  if (sercom->isDataRegisterEmptyUART() && txBuffer.available() == 0) {
    ...
    //pinPeripheral(uc_pinTX, g_APinDescription[uc_pinTX].ulPinType);
    // Pin should be allocated to SERCOM
    //sercom->enableUARTtx();
    sercom->writeDataUART(data);
  } else {
